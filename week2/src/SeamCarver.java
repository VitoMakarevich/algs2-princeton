...
public class SeamCarver {
  ...
  public SeamCarver(Picture picture) {
    checkNull(picture);
    this.pictureMatrix = new int[picture.width()][picture.height()][3];
    for (int i = 0; i < picture.width(); i++) {
      for (int j = 0; j < picture.height(); j++) {
        this.pictureMatrix[i][j][0] = picture.get(i, j).getRed();
        this.pictureMatrix[i][j][1] = picture.get(i, j).getGreen();
        this.pictureMatrix[i][j][2] = picture.get(i, j).getBlue();
      }
    }
  ...
  public Picture picture() {
    ...
    for (int i = 0; i < width; i++) {
      for (int j = 0; j < height; j++) {
        picture.set(i, j, new Color(this.pictureMatrix[i][j][0], this.pictureMatrix[i][j][1],
                                    this.pictureMatrix[i][j][2]));
      }
    }
  ...
  public double energy(int x, int y) {
    ...
    double xEnergyDiffPow2 = Math.pow(
        this.pictureMatrix[x - 1][y][0] - this.pictureMatrix[x + 1][y][0], 2
    ) +
        Math.pow(
            this.pictureMatrix[x - 1][y][1] - this.pictureMatrix[x + 1][y][1], 2) +
        Math.pow(
            this.pictureMatrix[x - 1][y][2] - this.pictureMatrix[x + 1][y][2], 2);
    double yEnergyDiffPow2 =
        Math.pow(this.pictureMatrix[x][y + 1][0] - this.pictureMatrix[x][y - 1][0], 2) +
            Math.pow(
                this.pictureMatrix[x][y + 1][1] - this.pictureMatrix[x][y - 1][1],
                2) +
            Math.pow(
                this.pictureMatrix[x][y + 1][2] - this.pictureMatrix[x][y - 1][2], 2);
  ...
  private EdgeWeightedDigraph constructVerticalDigraph() {
    ...
    for (int i = 0; i < this.width; i++) {
      ...
      // as numeration starts from 0, end item id is + 1
  ...
  // Convert x, y to one-dimensional id. Used in graphs where vertex is int.
  // Reverse of oneDimensionalToTwoDimensional
  private int twoDimensionalCoordToOneDimensional(int x, int y) {
    int result = x * this.height + y + 1;
    return result;
  }

  // Convert one-dimensional id to x, y. Used in graphs where vertex is int.
  // Reverse of twoDimensionalCoordToOneDimensional
  ...
  // Implemented by shifting all pixels after seam by 1 pixel. Complexity
  // is O(n).
